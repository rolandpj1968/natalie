...
SymbolObject *Object::define_singleton_method(Env *env, Value self, SymbolObject *name, LexicalScope *lexical_scope, MethodFnPtr fn, int arity, int break_point) {
    std::lock_guard<std::recursive_mutex> lock(g_gc_recursive_mutex);

    ClassObject *klass = singleton_class(env, self);
    if (klass->is_frozen())
        env->raise("FrozenError", "can't modify frozen object: {}", self.to_s(env)->string());
    klass->define_method(env, name, lexical_scope, fn, arity, break_point);
    return name;
}
...
SymbolObject *Object::define_method(Env *env, Value self, SymbolObject *name, LexicalScope *lexical_scope, MethodFnPtr fn, int arity, int break_point) {
    if (self.is_module())
        return self.as_module()->define_method(env, name, lexical_scope, fn, arity, break_point);

    if (GlobalEnv::the()->instance_evaling())
        return define_singleton_method(env, self, name, lexical_scope, fn, arity, break_point);

    self.klass()->define_method(env, name, lexical_scope, fn, arity, break_point);
    return name;
}
